...
import { PenIcon, ImageIcon, PlusIcon } from 'lucide-react';
...
const BusinessInfo = () => {
	...
	return (
		<div className='flex flex-col px-4 py-5 md:px-8'>
			...
			<div className='p-4 md:p-[3rem] pt-[1.5rem] lg:pt-[3rem] mt-[1rem] border-t-[1px] w-full flex'>
				<div className='flex flex-col lg:flex-row items-center w-full gap-[1.5rem] lg:gap-[5rem]'>
					{selectedBusiness?.logo ? (
						...
						/>
						<AvatarFallback><Skeleton className='h-[6rem] w-[6rem] rounded' /></AvatarFallback>
						</Avatar>
					) : (
					...
					<div className='w-full lg:w-[80%]'>
						<div className=' w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-5 md:gap-y-8 gap-x-8 justify-between'>
							...
							<div className='flex flex-col gap-[0.4rem]'>
								<div className='text-[0.95rem] sm:text-[1.1rem] text-gray-400'>Business Address</div>
								<h1 className='text-[1.05rem] sm:text-[1.2rem]'>{selectedBusiness?.address || 'Not Available'}</h1>
							</div>
							<div className='flex flex-col gap-[0.4rem]'>
							...
							<div className='flex flex-col gap-[0.4rem]'>
								...
								<h1 className='text-[1.05rem] sm:text-[1.2rem]'>{selectedBusiness?.phoneNumber || 'Not Available'}</h1>
							</div>
							<div className='flex flex-col gap-[0.4rem]'>
			...
			<Button onClick={businessModal.toggleEditOpen} className='self-start mx-auto bg-core hover:bg-blue-800 text-white'>
