...
import { Customer, Invoice, SuccessResponse } from '#/common.types';
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post<{ data: Invoice }>(
        `/api/invoices?businessId=${businessId}&customerId=${customerId}`,
        data
      );
        ...
        <div className="flex w-full  gap-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                className="flex justify-between w-full sm:max-w-[300px]"
              >
            ...
            <DropdownMenuContent
              align="start"
              className="w-[300px] shadow-lg p-1"
            >
              ...
              <DropdownMenuGroup className="overflow-y-auto h-[250px] flex flex-col items-center">
                {isPending ? (
                  <Loader
                    type="spinner"
                    size={36}
                    className="my-4 text-black dark:text-white leading-[0]"
                  />
          ...
          >
            <span className="flex md:hidden">
              <PlusIcon className="h-4 w-4 mr-0 sm:mr-2" />
            </span>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p>Customer Name</p>
              <p className="text-sm font-thin leading-none opacity-70">
      ...
      <Card className="flex flex-col mt-4">
              ...
              <TableRow className="hover:bg-transparent">
                ...
                <TableCell className="space-y-2">
                  <Input
                    ...
                    value={quantity}
                    onChange={(e) =>
                      setQuantity(Math.max(0, e.target.valueAsNumber))
                    }
                  />
                </TableCell>
                <TableCell className="space-y-2">
                  <Input
                    ...
                    value={price}
                    onChange={(e) =>
                      setPrice(Math.max(0, e.target.valueAsNumber))
                    }
                  />
                </TableCell>
                <TableCell className="space-y-2">
        ...
        <CardFooter className="flex items-center justify-between">
          ...
          <Button
            onClick={onAdd}
            size="sm"
            className="text-white bg-core hover:bg-blue-800"
          >
            Add Item
          </Button>
        </CardFooter>
      ...
      <Card className="flex flex-col mt-4 w-full sm:w-96 ml-auto">
        ...
        <CardContent className="flex items-center justify-between">
          ...
          <span className="text-lg font-semibold">
            {formatNumberWithCommas(overallTotal)}
          </span>
        </CardContent>
      </Card>
